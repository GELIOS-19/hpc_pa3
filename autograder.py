...
def run_pa3(kind, nranks, testfile):
    return subprocess.run(pa3_command(kind, nranks, testfile), capture_output=True, text=True)
...
def check_files(files):
    ...
    if not all(os.path.exists(f) for f in files):
        ...
        print("Please make sure that you are running the autograder script from the root of the project and that all files have been downloaded.", file=sys.stderr)
        sys.exit(1)


def correctness_checks():
    """Run correctness checks on smaller test files."""
    spgemm_testfiles = ["testfiles/ibm32-spgemm.dat", "testfiles/simple-spgemm.dat"]
    apsp_testfiles = ["testfiles/ibm32-apsp.dat", "testfiles/simple-apsp.dat"]
...
def perf_checks():
    ...
    perf_testfiles = {"spgemm": ("perf-testfiles/GL7d14.dat", {16: [20.0, 30.0], 36: [10.0, 20.0]}, [20, 10]), "apsp": ("perf-testfiles/G12.dat", {16: [65.0, 80.0], 36: [30.0, 40.0]}, [15, 10])}
